...
#                 print(ghost_nodes[i])
    ...
    for i in range(size):
        for j in range(len(cell_part[i])):
                cell_part[i][j] = [GlobNodeToLoc[i,cell_part[i][j][0]], GlobNodeToLoc[i,cell_part[i][j][1]], 
            ...
            for j in range(len(haloext[i])):
                if len(haloext[i][j]) > 0:
                    for k in range(len(haloext[i][j][0])):
                        text_file.write(str(cell_nodeid[haloext[i][j][0][k]][0])+" "+ str(cell_nodeid[haloext[i][j][0][k]][1])
